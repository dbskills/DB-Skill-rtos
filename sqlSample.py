...
# Pre-trained alias->embedding-index map (JOB benchmark). Used as a base for the
# dynamic alias map so pre-trained embeddings are reused for known aliases.
_PRETRAINED_ALIAS_ID2NAME = {0: 'start', 1: 'chn', 2: 'ci', 3: 'cn', 4: 'ct', 5: 'mc', 6: 'rt', 7: 't', 8: 'k', 9: 'lt', 10: 'mk', 11: 'ml', 12: 'it1', 13: 'it2', 14: 'mi', 15: 'mi_idx', 16: 'it', 17: 'kt', 18: 'miidx', 19: 'at', 20: 'an', 21: 'n', 22: 'cc', 23: 'cct1', 24: 'cct2', 25: 'it3', 26: 'pi', 27: 't1', 28: 't2', 29: 'cn1', 30: 'cn2', 31: 'kt1', 32: 'kt2', 33: 'mc1', 34: 'mc2', 35: 'mi_idx1', 36: 'mi_idx2', 37: 'an1', 38: 'n1', 39: 'a1'}

# Persisted alias->id map. Loaded by the wrapper at startup (from state file).
# New aliases encountered at runtime are added here so the same alias always
# maps to the same embedding index across invocations. The wrapper saves this
# dict back to disk after each invocation.
_PERSISTED_ALIAS_MAP = {}

def set_persisted_alias_map(m):
    """Called by the wrapper to inject the persisted alias->id map."""
    global _PERSISTED_ALIAS_MAP
    _PERSISTED_ALIAS_MAP = dict(m)

def get_persisted_alias_map():
    """Called by the wrapper to read the (possibly extended) alias map for saving."""
    return dict(_PERSISTED_ALIAS_MAP)

def _build_alias_maps(aliases):
    """Build (id2aliasname, aliasname2id) from the persisted map, extending it
    for new aliases. The model architecture (join_matrix 40x40, nn.Embedding
    n_words=100) caps alias ids at 0-39 for join_matrix indexing. New aliases
    beyond the 39-slot capacity fall back to id 0 ('start')."""
    aliasname2id = dict(_PERSISTED_ALIAS_MAP)
    # Determine next free id in [1, 39]
    used = set(aliasname2id.values())
    next_id = 1
    for alias in sorted(aliases):
        if alias in aliasname2id:
            continue
        while next_id <= 39 and next_id in used:
            next_id += 1
        if next_id <= 39:
            aliasname2id[alias] = next_id
            used.add(next_id)
            next_id += 1
        else:
            aliasname2id[alias] = 0  # 'start' fallback for overflow
    # Update the persisted map so the wrapper can save it.
    _PERSISTED_ALIAS_MAP.update(aliasname2id)
    id2aliasname = {0: 'start'}
    for a, i in aliasname2id.items():
        if i != 0:
            id2aliasname[i] = a
...
class JoinTree:
    def __init__(self,sqlt,db_info,max_column_in_table = 15,max_alias = 40,extent_sql = True):
        ...
        self.id2aliasname, self.aliasname2id = _build_alias_maps([t.getAliasName() for t in self.from_table_list])
        ...
        _wc = parse_result["whereClause"]
        if "BoolExpr" in _wc:
            _wc_args = _wc["BoolExpr"]["args"]
        else:
            _wc_args = [_wc]
        self.comparison_list = [Comparison(x) for x in _wc_args]
